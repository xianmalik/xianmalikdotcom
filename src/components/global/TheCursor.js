...
const TheCursor = () => {
	...
	return (
		<>
			...
			<span
				style={{ left: `${x}px`, top: `${y}px` }}
				className="fixed top-0 left-0 h-12 w-12 border-2 border-white opacity-25 border-opacity-75 rounded-full -translate-x-1/2 -translate-y-1/2 transform z-50 transition-all duration-200 ease-out pointer-events-none"
			></span>
			<span
				className="fixed top-1/2 left-1/2 w-6 h-6 bg-white rounded-full opacity-25 -translate-x-1/2 -translate-y-1/2 transform z-50 pointer-events-none"
				style={{ left: `${x}px`, top: `${y}px` }}
			></span>
		</>
	);
